...
//
// adapatado para Lotuspay por t.me/drcloud8
//
...
try {
    ...
    $apiSecret = 'lp_0b4fec2a449f7a2765695d650ff0df682ed23075cc7016fe';
    ...
    error_log("[Pagamento] ✅ Dados do usuário recebidos: " . json_encode($userData));
    ...
    error_log("[Lotuspay] 📝 Preparando dados para envio: " . json_encode([
        'valor' => $valor,
    ...
    $data = [
        ...
        "amount" => $valor_centavos / 100, // Convertendo centavos para reais
    ...
    error_log("[LotusPay] 💳 Transação criada com sucesso: " . $result['id']);
    error_log("[LotusPay] 📄 Resposta completa da API: " . $response);
    error_log("[LotusPay] 🔑 Token gerado: " . $result['id']);
    ...
    $pixCode = isset($result['qrCode']) ? $result['qrCode'] : null;
    $pixCodeUrl = isset($result['qrCode']) ? 'https://api.qrserver.com/v1/create-qr-code/?data=' . urlencode($result['qrCode']) . '&size=300x300' : null;
    $responseData = [
        'success' => true,
        'token' => $result['id'],
        'pixCode' => $pixCode,
        'qrCodeUrl' => $pixCodeUrl,
        'valor' => $valor,
        'userData' => $userData,
        'agendamento' => $agendamento,
        'logs' => [
    ...
    error_log("[LotusPay] 📤 Enviando resposta ao frontend: " . json_encode($responseData));
    echo json_encode($responseData);

} catch (Exception $e) {
    error_log("[LotusPay] ❌ Erro: " . $e->getMessage());
    error_log("[LotusPay] 🔍 Stack trace: " . $e->getTraceAsString());
