...
try {
    ...
    $apiUrl = 'https://api.genesys.finance/v1';
    $apiSecret = 'sk_0621938c47a7d9cad9f6f2250a3e792973e5aad363544e074fbc0c1f4c2ec6568732a07cb51a3e28d21dce697d18fad317f9c1963f8f7cb67a7ea3963202c5e9';
    ...
    $valor = isset($_POST['valor']) ? intval($_POST['valor']) : 8724; // Valor dinâmico ou padrão
    ...
    $webhookUrl = "https://" . $_SERVER['HTTP_HOST'] . "/consulta/checkout/webhook.php";
    ...
    error_log("[NovaAPI] 🌐 URL da requisição: " . $apiUrl . '/transactions');
    error_log("[NovaAPI] 📦 Dados enviados: " . json_encode($data));
    ...
    error_log("[NovaAPI] 🔍 Detalhes da requisição cURL:\n" . $verboseLog);

    if ($curlError) {
        error_log("[NovaAPI] ❌ Erro cURL: " . $curlError . " (errno: " . $curlErrno . ")");
        throw new Exception("Erro na requisição: " . $curlError);
    }

    curl_close($ch);

    error_log("[NovaAPI] 📊 HTTP Status Code: " . $httpCode);
    error_log("[NovaAPI] 📄 Resposta bruta: " . $response);
    ...
    $utmifyUrl = $serverUrl . "/consulta/checkout/utmify-pendente.php";
